...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "sample-jakartaee-ear-archetype-ear.ear");

        // Current directory is the root of the "sample-jakartaee-ear-archetype-web" project. Go up one level
        // and enter the "sample-jakartaee-ear-archetype-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../sample-jakartaee-ear-archetype-ejb/target/sample-jakartaee-ear-archetype-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "sample-jakartaee-ear-archetype-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/sample-jakartaee-ear-archetype-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "sample-jakartaee-ear-archetype-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
